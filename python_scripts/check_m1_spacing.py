...
def check_m1_spacing(input_gds: str, output_markers_gds: str = None):
    ...
    if not violations:
        ...
        for i, v in enumerate(violations, 1):
            mx, my = v["midpoint"]
            print(f"  [{i:>4}]  spacing = {v['spacing_um']:.4f} µm  |  location ≈ ({mx}, {my}) µm")
            print(f"          edge1: ({v['edge1'][0]}, {v['edge1'][1]}) → ({v['edge1'][2]}, {v['edge1'][3]}) µm")
            print(f"          edge2: ({v['edge2'][0]}, {v['edge2'][1]}) → ({v['edge2'][2]}, {v['edge2'][3]}) µm")
    print("=" * 60)
